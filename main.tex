...
Red line shows \textbf{T} calculated numerically (\EQ{T}) by assuming that $P(\tau_{k} > 0) = 0$.  
%
...
%
Red lines show \textbf{T} calculated numerically (\EQ{T}) by assuming that $P(\tau_{k} > 0) = 0$ for 41 values of the parameter being manipulated, evenly spaced on a log-scale.
%
...
%
\textbf{(A)} Expected extinction time, $\mathbf{T}$, of populations founded by $n_0=100$ mutation-free individuals and subject to mutations with deleterious effect $s$ and rate $u$.  \textbf{T} was calculated numerically (\EQ{T}) by assuming that $P(\tau_{k} > 0) = 0$ for $21\times21=441$ combinations of values of $s$ and $u$ evenly spaced on a log-scale.
%
\textbf{(B)} Expected extinction time of the mutation-free class, $t_0$ (\EQ{t0}), as a proportion of \textbf{T} for the populations shown in \textbf{(A)}. 
%
...
\end{equation*}
%
We calculate \textbf{T} numerically (\EQ{T}) by assuming that $P(\tau_{k} > 0) = 0$ and computing \EQ{tk} until the following criterion is met 
\begin{equation*}
...
In our model, population size, $N$, can increase as well as decrease from generation to generation.  However, all increases are transient and the population will eventually go extinct (\FIG{decay}A).  The expected value of $N$ can be predicted accurately by \EQ{N} (\FIG{decay}B).  However, the dynamics of the expected value of $N$ are not sufficient to predict the time to extinction.  Two populations with different initial population sizes, $n_0$, will be expected to show the same $N/n_0$ at any time (because they will have the same $g_j(t)$, \EQ{N}), but the smaller population is expected to go extinct earlier (\FIG{decay}C and \ref{fig:decay}D).  \EQ{T} provides good estimates of extinction time, \textbf{T}, over a broad range of initial population sizes, $n_0$ (\FIG{decay}D), and mutational parameters (\FIG{valid}A and \ref{fig:valid}B).  
(See Appendix 2 for an explanation of why \EQ{T} tends to overestimate the true value of \textbf{T}.)  
Below, we focus on the numerical estimates of \textbf{T} based on \EQ{T}.

Our model has three parameters---$n_0$, $u$, and $s$---and all of them influence extinction time.  Smaller populations tend to go extinct faster.  \FIG{decay}D shows that \textbf{T} is approximately proportional to the logarithm of the initial population size, $n_0$.  
%
...
$t_0$ grows logarithmically, this result implies that $t_0$ represents an increasing fraction of \textbf{T} as $n_0$ increases. 
Therefore, we also expect \textbf{T} to grow logarithmically with $n_0$.


At a particular value of $n_0$, however, $t_0$ is not sufficient to explain the variation in total extinction time: 
$t_0$ dominates \textbf{T} when $u/s$ is low, but not when it is high (\FIG{heat}B).  One reason for this pattern is that $x_1$ increases with $u/s$ (\EQ{x1}; \FIG{heat}C).


Mutations cause extinction in our model, but how do they influence extinction \textit{time}?  
%
One possibility is that \textbf{T} is determined by the severity of Muller's Ratchet. High mutation rate $(u)$ and mutations of large effect $(s)$ accelerate extinction in doomed populations (\FIG{valid} and \FIG{heat}A).  Mutational parameters act differently on Muller's Ratchet in populations of constant size: at certain mutation rates the severity of the Ratchet is maximal at intermediate mutational effects \citep{Gabriel_MULLER_1993, gor00b, Gordo_On_2000} (\FIG{valid}C).
%
...
%
In populations of constant size, $\Delta t_k$ is not expected to change with $k$ \citep{Haigh_The_1978, gor00b, Gordo_On_2000}.  
%
...
\FIG{melt}A shows that doomed populations can experience strong mutational meltdown.  However, mutational meltdown does not determine extinction time.  Although the three scenarios summarized in \FIG{melt}A have widely different meltdown rates, they have the same expected extinction time, $\mathbf{T} = 223.52$.  The lack of correlation between \textbf{T} and $\beta$ is clear when we compare \FIG{heat}A and \FIG{melt}B.
