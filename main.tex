...
have identified two major risk factors that can drive populations into the doomed regime: long-term reductions in population size and increases in mutation rate   \citep{Lynch_MUTATION_1990, lyn93, Gabriel_MULLER_1993, lyn95}.  Population size could decline as a result of changes in the environment, such as, climate change, decreased food availability, habitat loss or fragmentation, 
\red{and emergence of infectious diseases.  
For example, the emergence of Devil Facial Tumour Disease, a transmissible cancer, has caused the size of the Tasmanian devil population to decline by $\sim 77\%$ within 5 years \citep{haw06, laz18}.  Devils are under risk of extinction \citep{mcc09}.} 
%in the United Kingdom, the population sizes of 10 species of seed-eating birds have declined because of the use of herbicides and the shift from spring-sown to fall-sown cereals, both of which have reduced the availability of food for these birds \citep{new04}.
Increases in mutation rate have been observed directly in experimental populations.  For example, a population of \emph{Escherichia coli} adapting to a constant environment evolved a mutator mutation at $\sim 2.5 \times 10^4$  generations that increased mutation rate by $\sim 150$-fold \citep{bar09, wie13}.
...
Evolution experiments have found evidence that real populations can experience  increased extinction risk when the mutation rate is high \citep{zey01a, ban16}.  
...
\begin{itemize}
\item \sout{Although drift is irrelevant in driving extinction, it nevertheless drives a population into the meltdown regime via Ratchet clicks in the Haigh regime}
%
\item \sout{Lynch and Gabriel (1990), using simulations, also find that extinction time is a function of $us$.}  
%
\item Log dependence of extinction time on $n_0$ implies that large increases in $n_0$ cause only relatively small delays in ultimate extinction.   Similar to a result in other stochastic models of population dynamics \citep{lan93, Jagers_On_2007}.  Lynch et al. (1993) also infer that for extinction time during the meltdown phase based on simulations.
%
...
%
\item \sout{Relative vs absolute fitness.}
%
...
%
\item \sout{Lethal mutagenesis.  They also ignore the meltdown phase.}
%
...
%  \label{fig:sim}
%\end{figure}
As shown in the Theory section, an approximation is made in \EQ{approx} in order to derive analytical expressions for click times ($t_k$) and time to extinction (\textbf{T}). 
