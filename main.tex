...
Red line shows $\mathbf{T}$ calculated numerically based on \EQ{T} (see \textbf{Methods and Materials}).  
%
...
%
Blue circles show coefficient of variation of $T$, CV$[T]$, for the data shown in \FIG{decay}D.  Red line shows CV$[T]$ calculated numerically using \EQ{CV} (see \textbf{Methods and Materials}).}{\includegraphics[width=0.33\linewidth]{decay2}}\label{figsupp:sf1}
\end{figure}
...
\end{equation}
%
When $k>0$, \EQ{tk} is an approximation (\EQ{tauk} and \EQ{xk}).
...
%
Light blue regions indicate $\overline{T} \pm  \mathrm{SD}[T]$  If $\mathrm{SD}[T] > \overline{T}$, the lower bound of the region was set to zero (see \FIGSUPP[valid]{sf2} for more on the variability of $T$).
%
Red lines show $\mathbf{T}$ calculated numerically based on \EQ{T} (see \textbf{Methods and Materials}) for 41 values of the parameter being manipulated, evenly spaced on a log-scale.
%
...
%
Blue circles show coefficient variation of $T$, CV$[T]$, for the data shown in the corresponding panel of \FIG{valid}.  Red line shows CV$[T]$ calculated numerically using \EQ{CV}  (see \textbf{Methods and Materials}).}{\includegraphics[width=0.67\linewidth]{valid2}}\label{figsupp:sf2}
\end{figure}
...
%
\textbf{(A)} Expected extinction time, $\mathbf{T}$, of populations founded by $n_0=100$ mutation-free individuals and subject to mutations with deleterious effect $s$ and rate $u$.  $\mathbf{T}$ was calculated numerically based on \EQ{T} (see \textbf{Materials and Methods}) for $21\times21=441$ combinations of values of $s$ and $u$ evenly spaced on a log-scale.  (See \FIGSUPP[heat]{sf3}B for the variability of $T$.)
%
\textbf{(B)} Expected extinction time of the mutation-free class, $t_0$ (\EQ{t0}), as a proportion of $\mathbf{T}$ for the populations shown in \textbf{(A)}.  (See \FIGSUPP[heat]{sf3}A for $t_0$.)
%
\textbf{(C)} Expected number of individuals with $k=1$ mutation at $t_0$, $x_1$ (\EQ{xk}), for the populations shown in \textbf{(A)}.
}
\label{fig:heat}
\figsupp[Variability of extinction time and extinction time of the mutation-free class.]{\textbf{Variability of extinction time and extinction time of the mutation-free class.}
%
\textbf{(A)} Coefficient of variation of $T$, CV$[T]$, calculated numerically using \EQ{CV} (see \textbf{Materials and Methods}).
%
\textbf{(B)} Expected extinction time of the mutation-free clas, $t_0$, used in \FIG{heat}B.}{\includegraphics[width=0.67\linewidth]{heat2}}\label{figsupp:sf3}
\end{figure}
...
\red{Variability in extinction time declines with $n_0$ (\FIGSUPP[decay]{sf1}).}
...
$t_0$ dominates $\mathbf{T}$ when $u/s$ is low, but not when it is high (\FIG{heat}B and \FIGSUPP[heat]{sf3}A).  One reason for this pattern is that $x_1$ increases with $u/s$ (\EQ{x1}; \FIG{heat}C).
...
\red{For a given extinction time, the meltdown rate is positively related to the \textit{variability} in extinction time (\FIGSUPP[valid]{sf2} and \FIGSUPP[heat]{sf3}B).}
...
Our results challenge the Lynch-Gabriel hypothesis.  Although doomed populations can experience mutational meltdown---measured by the acceleration of the Muller's Ratchet---, the rate of mutational meltdown does not determine extinction time (\FIG{melt}A).  Therefore, the Lynch-Gabriel mutational meltdown is not a general \textit{cause} of extinction.  Rather, our results suggest that extinction time is determined by $us$, the expected impact of deleterious mutations on fitness.  
\red{Interestingly, if we compare populations with the same expected extinction time but different mutational parameters, populations undergoing faster meltdown rate have more variable extinction times than populations undergoing slower meltdown  (\FIGSUPP[heat]{sf3}).}
...
\end{equation*}
%
A similar criterion was applied when evaluating \EQ{tk} and \EQ{vartauk}. 
...
\end{equation*}
%
\red{We computed $\mathbf{T}$ for both bounds of $P(\tau_{k} > 0)$ when $k>0$ (\EQ{bounds}).  We present only results for the lower bound.  None of our conclusions would be changed by using the upper bound results instead (not shown).}
...
\red{Again, we only present results for the lower bound of $P(\tau_{k} > 0)$ when $k>0$ (\EQ{bounds}).}
