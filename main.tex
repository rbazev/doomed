...
\blue{and cancer progression \citep{McFarland_Tug_2014}}.  
...
Beginning with Haigh's foundational study, most research on Muller's Ratchet (including all the papers \blue{cited in the previous paragraph}) has assumed that 
\blue{the size of a population remains constant as deleterious mutations accumulate.}  
...
However, they did not present any analytical \blue{theory} on the dynamics or duration of the meltdown (third) phase itself \citep[see also][]{Gabriel_MULLER_1993, lyn95}.
\blue{Thus, the validity of the Lynch-Gabriel model of mutational meltdown is difficult to evaluate.}
%%%
...
Thus, 
\blue{$\Delta t_1, \Delta t_2, \ldots$ (\EQ{deltat})}
...
\subsection{\blue{Low mutation rate and mutations of large effect accelerate mutational meltdown}}
...
\blue{Do doomed populations experience mutational meltdown?}
...
\blue{the Ratchet is expected to click at a steady rate over time, that is,}
...
\blue{the Ratchet is expected to accelerate in successive clicks, that is,}
...
\blue{Mutational parameters have large effects on the rate of mutational meltdown.  Low mutation rate and mutations of large effect accelerate mutational meltdown (\FIG{melt}).  This result is consistent with the effects of mutational parameters on $\Delta t_0$ and $\Delta t_1$ in large populations (\EQ{jagers} and \EQ{x1}).  
%
Low values of $u$ increase $\Delta t_0$  and decrease $x_1$, and therefore $\Delta t_1$, causing the mutational meltdown to accelerate.  
%
...
However, the assumption is unrealistic because it actually prevents populations from ever going extinct \citep{Lynch_MUTATION_1990, mel91}.  
%
...
Our results challenge the Lynch-Gabriel hypothesis.  Although doomed populations can experience mutational meltdown---measured by the acceleration of the Muller's Ratchet---, the rate of mutational meltdown does not determine extinction time (\FIG{melt}A).  Therefore, the Lynch-Gabriel mutational meltdown cannot be a general \textit{cause} of extinction.  Rather, our results suggest that extinction time is determined by $us$, the expected impact of deleterious mutations on fitness.  
%
The Lynch-Gabriel hypothesis emphasized the role of the change in the strength of genetic drift in causing mutational meltdown \citep[e.g., ``we refer to this synergism between mutation accumulation and random genetic drift as a mutational meltdown'';][]{lyn93}.  Our results indicate that extinction is driven by mutation pressure, not genetic drift.  
%
We propose that the expression mutational meltdown \citep[now in common usage; e.g.,][]{Allen_Mutational_2009, McFarland_Tug_2014} be redefined to refer to extinction caused by mutation pressure.

Evolution experiments have found evidence that real populations can experience  increased extinction risk when the mutation rate is high \citep{zey01a, ban16}.  For example, \citet{ban16} subjected populations of influenza A virus to high concentrations of favipiravir, a drug that increases the mutation rate of the virus, and observed that populations accumulated mutations rapidly and went extinct. 
These experimental results are consistent with our interpretation of the mutational meltdown because increasing mutation rate causes faster extinction in our model.

The relative theoretical neglect of the time to extinction of a doomed population (i.e., \textit{during} mutational meltdown) can probably be attributed to the expectation that it is quick relative to the time required for the population to become doomed \citep{lyn93, lyn95, Bull_Theory_2007}.  We believe this neglect is misplaced because the mutational meltdown phase is crucial for the fate of the population.  It is the last chance for the population to be rescued by beneficial mutations and escape extinction.  




\subsection{Other potential topics}

Below is a haphazard collection of topics to consider discussing.  Feel free to add some bullets.  I've crossed out things that we have already mentioned.
...
%
\item \sout{Increasing $s$ accelerates extinction, counter the results of Lynch et al., who found that intermediate $s$ minimizes extinction time.}  
%
\item \sout{Extinction does not necessarily imply meltdown even if the two are used interchangeably.}
%
...
%
\item \sout{The doomed regime is intrinsically interesting because it constitutes the last chance for a population to be rescued by compensatory mutations.}  For example, this is relevant for the case of cancer.  And for the evolution of sex where asexuals are thought to be more susceptible to extinction.
%
...
%
\item Lethal mutagenesis.  \sout{They also ignore the meltdown phase.}
%
...
We thank Alex Stewart for helpful comments.  
...
\label{app:jensen}
%\begin{figure}[h!]
  %\includegraphics[width=0.5\textwidth]{TvsuMay28.pdf}
%  \caption{Simulation and theoretical predictions of time to extinction $(T)$ for a range of mutation rates. Selection coefficient $(s)$ is $0.01$, and initial population size $(n_{0})$ is $100$ for all runs. Mean simulation results, taken from $10,000$ runs, are shown as circles, while analytical predictions are shown as triangles. Simulation standard deviations are shown as vertical lines. Theory is shown to be quite accurate, although the inaccuracy is consistently an overestimation. This is explained by a property of concave functions, namely, that $E[f(X)]<f(E[X])$.}
%  \label{fig:sim}
%\end{figure}
As shown in the Theory section, an approximation is made in Equation \ref{eq:approx} in order to derive analytical expressions for click times ($t_k$) and time to extinction (\textbf{T}). 
%That is, $$E[m_{0,k+j}^{(T_{k})}]\approx g_{k+j}(t_{k}).$$
%In order to check that this approximation holds, we ran simulations of population runs according to the presented model. In each generation, every individual either splits or dies, and every offspring acquires a new mutation with probability $u$. A run ends when the population size is zero. 
...
Overestimation of \textbf{T} is partly explained by the concavity of $E[\tau_k]$ as a function of $x_k$ in Equation \ref{eq:tauk}. %(having discussed the extinction of consecutive classes, we now replace the original $n_{k}$ in Equation \ref{eq:tauk} with $x_k$). 
